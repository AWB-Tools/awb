 ...
 * @file dralListener.h
 * @author Pau Cabre 
 * @brief dral listener interface
 */

/**
  * @example dralListenerExample
  * This listener example shows us the code of a listener that only cares about
  * AddNodes, AddEdges, NewItems, DeleteItems and MovingItems.
  * All the other callbacks are ignored.
  */

#ifndef _DRALLISTENER_H
#define _DRALLISTENER_H

#include "asim/dral_syntax.h"

/**
  * @brief Class used to listen the different events
  *
  * This pure virtual class contains all the methods the user must implement
  * in order to be able to process the DRAL events.
  * The DRAL_CLIENT_CLASS::ProcessNextEvent method will call these different
  * methods acording to the events it will read.
  * Since the Dral Client hast to able to process events of DRAL 1.0
  * and DRAL 2.0, one will find three kinds of methods in this class:
  *   - CallBacks for commands found in both DRAL 1.0 and DRAL 2.0
  *   - CallBacks for DRAL 2.0 commands that are not found in DRAL 1.0
  *   - CallBacks for commands only found in DRAL 1.0
  * Those later callbacks will be only invoked when processing DRAL 1.0 events
  */
class DRAL_LISTENER_CLASS
{
  public:
    ...
    /**
      ...
      */
    virtual void SetCycleTag(UINT32 tag_idx, UINT64 value)=0;
    ...
    /**
      ...
      */
    virtual void SetCycleTagString(UINT32 tag_idx, UINT32 str_idx)=0;
    ...
    /**
      ...
      * @param set The set
      */
    virtual void SetCycleTagSet(UINT32 tag_idx, UINT32 nval, UINT64 set [])=0;
    ...
    virtual void SetItemTag(
        UINT32 item_id, UINT32 tag_idx, UINT64 value)=0;
    ...
    virtual void SetItemTagString(
        UINT32 item_id, UINT32 tag_idx, UINT32 str_idx)=0;
    ...
    virtual void SetItemTagSet(
        UINT32 item_id, UINT32 tag_idx, UINT32 nval, UINT64 set [])=0;
    ...
    */
    virtual void NewClock (
        UINT16 clockId, UINT64 freq, UINT16 skew,
        UINT16 divisions, const char name [])=0;
    ...
    virtual void SetTagString (
        UINT32 item_id, UINT32 tag_idx,
        UINT32 str_idx, UBYTE time_span_flags)=0;
    ...
    /**
      ...
      */
    virtual void NewTag (
        UINT32 tag_idx, const char * tag_name, INT32 tag_name_len)=0;
    ...
    virtual void NewString (
        UINT32 string_idx, const char * str, INT32 str_len)=0;

    /*@}*/ // End of the block: CallBacks for commands only found in DRAL 1.0

};
typedef DRAL_LISTENER_CLASS * DRAL_LISTENER; /**<Pointer type to
                                               * a DRAL_LISTENER_CLASS
                                               */

#endif /* _DRALLISTENER_H */
