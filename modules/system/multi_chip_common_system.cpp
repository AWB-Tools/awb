...
static ASIM_MULTI_CHIP_SYSTEM common_system = NULL;
...
ASIM_MULTI_CHIP_SYSTEM_CLASS::ASIM_MULTI_CHIP_SYSTEM_CLASS(
    ...
    : ASIM_SYSTEM_CLASS(name, TOTAL_NUM_CPUS * NUM_CHIPS, NULL, feederThreads),
      ...
      statBaseCycles(0)
{
    ...
    RegisterState(&statClocks, "clocks_stats_gathered", "Number of calls to clockserver's Clock with stats on");
    RegisterState(&statCycles, "cycles_stats_gathered", "Number of cycles simulated with stats on (@ reference frequency)");
    RegisterState(&statBaseCycles, "base_cycles_stats_gathered", "number of base cycles simulated with stats on (@ clockserver frequency)");
...
ASIM_MULTI_CHIP_SYSTEM_CLASS::DumpStats(STATE_OUT state_out)
{
    ...
    state_out->AddScalar("uint", "cycles_simulated",
                         "number of cycles simulated (@ reference frequency)", SYS_Cycle());
    
    state_out->AddScalar("uint", "cycles_stats_gathered",
                         "number of cycles simulated with stats on (@ reference frequency)", statCycles);

    state_out->AddScalar("uint", "base_cycles_simulated",
                         "number of base cycles simulated (@ clockserver frequency)", SYS_BaseCycle());

    state_out->AddScalar("uint", "base_cycles_stats_gathered",
                         "number of cycles simulated with stats on (@ clockserver frequency)", statBaseCycles);
    ...
    myBoard.DumpStats(state_out, SYS_Cycle(), SYS_CommittedInsts());
    ...
    const UINT64 stop_packet)
{
    ...
    T1("Executing until cycle " << stop_cycle << " or inst " << stop_inst << " or nanosecond " << stop_nanosecond); 
           ...
           (SYS_CommittedMarkers() < stop_marker)) 
    {
        ...
        statBaseCycles += bf_cycle_increment;
        statCycles += (sys_cycle - prevRefCycle);
    ...
    {
        cout << "WARNING: cycle " << SYS_Cycle() << ": " << SYS_CommittedMarkers() - start_marker << 
            " markers have been commited - scheduled only " << stop_marker - start_marker << endl;
    }
