 ...
 */
extern bool CMD_Init (UINT32 fdArgc, char **fdArgv, UINT32 pmArgc, char **pmArgv, char **allEnvp);
...
class CMD_INIT_CLASS : public CMD_WORKITEM_CLASS
{
    ...
    public:
         ...
         */
        void Schedule (CMD_SCHEDULE schedule, UINT64 currentNanosecond, UINT64 currentCycle, UINT64 currentInst, UINT64 currentPacket) {
            VERIFYX(false);
        }
...
class CMD_EXECUTE_CLASS : public CMD_WORKITEM_CLASS
{
    ...
    public:
        CMD_EXECUTE_CLASS (UINT64 n, UINT64 c, UINT64 i, UINT64 p) :
            CMD_WORKITEM_CLASS("EXECUTE"), nanoseconds(n), cycles(c), insts(i), packets(p) { }
