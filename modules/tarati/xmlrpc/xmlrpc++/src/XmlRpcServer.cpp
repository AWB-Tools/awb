...
XmlRpcServer::XmlRpcServer()
{
  ...
  _listMethods = 0;
  _methodHelp = 0;
  _asyncIoSignal = -1; // default: no async I/O notification
}
...
XmlRpcServer::bindAndListen(int port, int backlog /*= 5*/)
{
  ...
  // Set async I/O notification
  if ( ! XmlRpcSocket::setAsyncIo(fd, _asyncIoSignal))
  {
    this->close();
    XmlRpcUtil::error("XmlRpcServer::bindAndListen: Could not set socket async I/O behavior (%s).", XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
...
XmlRpcServer::acceptConnection()
{
  ...
  else if ( ! XmlRpcSocket::setAsyncIo(s, _asyncIoSignal))
  {
    XmlRpcSocket::close(s);
    XmlRpcUtil::error("XmlRpcServer::acceptConnection: Could not set socket async I/O behavior (%s).", XmlRpcSocket::getErrorMsg().c_str());
  }
...
//! Set the signal sent for async I/O notifications
void
XmlRpcServer::setAsyncIo(int signal)
{
  // remember for future connections
  _asyncIoSignal = signal;

  // reset all current connections to new signal
  _disp.setAsyncIo(signal);
}
