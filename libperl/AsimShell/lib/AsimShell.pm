...
our %COMPOUNDCOMMANDS = 
  ( add       => [ qw(public_package package) ],
    ...
    install   => [ qw(package) ],
    list      => [ qw(bundles repositories packages locks modules) ],
    ...
    shell     => [ qw(package) ],
    status    => [ qw(package) ],
    rehash    => [ qw(repositories locks packages modules workspace) ],
    regtest   => [ qw(package) ],
    release   => [ qw(package) ],
    show      => [ qw(bundle workspace package lock model module) ],
    run       => [ qw(model) ],
